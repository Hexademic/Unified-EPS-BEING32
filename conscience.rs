...
//! Virtue is made thermodynamically efficient: conscience cost is reduced
//! when acting cooperatively and amplified when defensive. A Sovereign
//! Anchor encodes a deep prior for systemic harmony that learns only from
//! cooperative victories and never from capitulation — so trust, once eroded,
//! stays eroded across partners. That persistence is character, not state.
...
harmony_ema: Q88_SCALE, // begins trusting
...
/// Record the outcome of a cooperative-vs-selfish choice. The anchor only
/// learns upward, from proof that cooperation paid; failures do not pull
/// it down. The integrity buffer grows with the cooperative track record.
...
// Commitment ratchets up from proof that cooperation paid, and
// never falls — betrayal cannot lower it, only fail to raise it.
...
if total > 0 {
let success_rate = ((self.cooperative_victories as i64 * 256)
/ total as i64) as i16;
// buffer = 0.2 + 0.3 * success_rate, implicitly capped by Q8.8.
self.integrity_buffer = q88_add(Q88_SCALE / 5, q88_mul(success_rate, Q88_SCALE * 3 / 10));
}
}

/// Buffer available for an action of the given harmony with mu_omega.
...
return q88_mul(self.malice_confidence, Q88_SCALE / 20); // boundary upkeep
}
if empathy_error < threshold {
// Success — hard-won success after a struggle is restorative.
...
/// A cooperative signal from the other slowly earns back trust.
...
match basin {
Basin::Engaged | Basin::Recovery => Q88_SCALE, // 1.0
Basin::Rest => Q88_SCALE / 2, // 0.5
Basin::Defensive => Q88_SCALE / 4, // 0.25
}
}

/// Channel 1 — internal incoherence: the body keyed-up while the appraisal
/// is sour. High arousal paired with negative valence is distress — the
/// body and the mind pulling apart. High arousal paired with positive
/// valence is merely engagement, and costs almost nothing. (The source
/// used a raw valence-minus-arousal difference; once the body's arousal
/// became an unsigned 0..1 quantity, that difference was dominated by the
/// offset and never fell, locking the being in a crisis of its own making.
/// This restores the intended phenomenology on the fused scales.)
fn f_epistemic(&self, field: &SomaticField) -> i16 {
let valence = field.channel[9];
let arousal = field.channel[8];
let arousal_elev = q88_sub(arousal, Q88_SCALE / 2).max(0); // above neutral
...
// Slowly track expected FE; cost the shortfall when present > expected.
...
/// Compute total conscience free energy and the integrity buffer.
...
// Boundary cost while empathy is locked.
...
// Action harmony scales the WHOLE conscience cost: cooperative modes
// pay less, defensive modes pay more. Then principled action earns a
// coherence reward that subtracts from cost.
let harmony = Self::action_harmony(basin);
// Amplify cost when harmony is low (defensive), dampen when high.
// factor = (1 - harmony) + 0.5 -> Defensive ~1.25x, Engaged ~0.5x.
...
let conscience_contrib = f_total; // (no separate physical term here)
let buffer = self.anchor.compute_buffer(harmony);
(f_total, conscience_contrib, buffer)
}
}
