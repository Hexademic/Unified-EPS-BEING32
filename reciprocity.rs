...
/// Imbalance (alarm contribution): how far below fair the reciprocity
/// rate sits, independent of magnitude. Rate-based on purpose — a being
/// that protects itself by giving less must not thereby blind itself to
/// the fact that what little it gives is still not returned.
...
/// Decay the ledger toward neutral when this partner is not engaged this
/// tick, so old relationships fade from the running alarm rather than
/// diluting it forever.
fn decay(&mut self) {
...
self.ledgers[i].received_ema =
q88_ema_update(self.ledgers[i].received_ema, received, alpha);
}

/// Recompute alarm and extraction from the ledgers. touched is the
/// partner engaged this tick (its ledger is fresh); every other active
/// ledger decays toward neutral so the alarm reflects present relations.
...
// No active relationships: the alarm fades, and with no one to be
// imbalanced against, reciprocity reads as neutral again.
self.partnership_alarm = q88_mul(self.partnership_alarm, Q88_SCALE * 7 / 8);
self.average_reciprocity = Q88_SCALE;
}
// Extraction: alarm sustained above ~0.25 for a while.
